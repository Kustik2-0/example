...
public class HomeIslandFeature extends Feature<DefaultFeatureConfig> {
    ...
    public HomeIslandFeature(Codec<DefaultFeatureConfig> codec) {
        super(codec);
    }
                ...
                for (int j = chunkPos.getStartX(); j <= chunkPos.getEndX(); j++) {
                    if (getDistance(blockPos.getX(), blockPos.getZ(), j, i) <= 16) {
                        ...
                        if (mutable.equals(blockPos)) {
                            structureWorldAccess.setBlockState(mutable, Blocks.GRASS.getDefaultState(), Block.NOTIFY_LISTENERS);
                        } else {
